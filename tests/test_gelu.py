...
def launch_gelu(dev, variant, x):
  ...
  kernel_name = f"gelu_kernel_BLOCK_SIZE_{BLOCK_SIZE}"
...
def test_gelu_basic(runtime_and_device, variant):
  _, dev = runtime_and_device
  x = torch.randn(4096, dtype=torch.float32)
  output = launch_gelu(dev, variant, x)
  expected = F.gelu(x)
  max_diff = (output - expected).abs().max().item()
  print(f"\nGELU basic BLOCK={variant['BLOCK_SIZE']}: max diff = {max_diff:.6e}")
  assert torch.allclose(output, expected, rtol=1e-5, atol=1e-5), f"Max diff: {max_diff:.6e}"
  print("✅ PASSED")

@pytest.mark.parametrize("variant", VARIANTS, ids=lambda v: f"BLOCK_{v['BLOCK_SIZE']}")
def test_gelu_all_negative(runtime_and_device, variant):
  _, dev = runtime_and_device
  x = -torch.abs(torch.randn(2048, dtype=torch.float32))
  output = launch_gelu(dev, variant, x)
  expected = F.gelu(x)
  max_diff = (output - expected).abs().max().item()
  print(f"\nGELU all-negative BLOCK={variant['BLOCK_SIZE']}: max diff = {max_diff:.6e}")
  assert torch.allclose(output, expected, rtol=1e-5, atol=1e-5), f"Max diff: {max_diff:.6e}"
  print("✅ PASSED")

@pytest.mark.parametrize("variant", VARIANTS, ids=lambda v: f"BLOCK_{v['BLOCK_SIZE']}")
def test_gelu_all_positive(runtime_and_device, variant):
  _, dev = runtime_and_device
  x = torch.abs(torch.randn(2048, dtype=torch.float32)) + 0.01
  output = launch_gelu(dev, variant, x)
  expected = F.gelu(x)
  max_diff = (output - expected).abs().max().item()
  print(f"\nGELU all-positive BLOCK={variant['BLOCK_SIZE']}: max diff = {max_diff:.6e}")
  assert torch.allclose(output, expected, rtol=1e-5, atol=1e-5), f"Max diff: {max_diff:.6e}"
  print("✅ PASSED")

@pytest.mark.parametrize("variant", VARIANTS, ids=lambda v: f"BLOCK_{v['BLOCK_SIZE']}")
def test_gelu_zeros(runtime_and_device, variant):
  _, dev = runtime_and_device
  x = torch.zeros(1024, dtype=torch.float32)
  output = launch_gelu(dev, variant, x)
  print(f"\nGELU zeros BLOCK={variant['BLOCK_SIZE']}")
  assert torch.all(output == 0.0), "GELU of zeros should be zero"
  print("✅ PASSED")

@pytest.mark.parametrize("variant", VARIANTS, ids=lambda v: f"BLOCK_{v['BLOCK_SIZE']}")
def test_gelu_odd_size(runtime_and_device, variant):
  _, dev = runtime_and_device
  x = torch.randn(1337, dtype=torch.float32)
  output = launch_gelu(dev, variant, x)
  expected = F.gelu(x)
  max_diff = (output - expected).abs().max().item()
  print(f"\nGELU odd-size (n=1337) BLOCK={variant['BLOCK_SIZE']}: max diff = {max_diff:.6e}")
  assert torch.allclose(output, expected, rtol=1e-5, atol=1e-5), f"Max diff: {max_diff:.6e}"
  print("✅ PASSED")

@pytest.mark.parametrize("variant", VARIANTS, ids=lambda v: f"BLOCK_{v['BLOCK_SIZE']}")
def test_gelu_large(runtime_and_device, variant):
  _, dev = runtime_and_device
  x = torch.randn(1024 * 1024, dtype=torch.float32)
  output = launch_gelu(dev, variant, x)
  expected = F.gelu(x)
  max_diff = (output - expected).abs().max().item()
  print(f"\nGELU large (1M) BLOCK={variant['BLOCK_SIZE']}: max diff = {max_diff:.6e}")
  assert torch.allclose(output, expected, rtol=1e-5, atol=1e-5), f"Max diff: {max_diff:.6e}"
  print("✅ PASSED")
